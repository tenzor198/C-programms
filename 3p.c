...
int Jordan(int n,double *a, double *b, double *x, int *index, int my_rank, int total_threads, int *t);
...
void *Solution(void *p_arg)
{
	...
	Jordan(arg->n,arg->A, arg->b,arg->x,arg->c, arg->my_rank, arg->total_threads, arg->t);
...
int Jordan(int n,double *a, double *b, double *x, int *index, int my_rank, int total_threads, int *t)
{
	...
	for (i = 0; i < n; i++)
	{
		...
		if(*t==1) {
			if(my_rank==0) printf("Вырожденная\n\n");
			return 0;
		}
		first_row = i * my_rank;
